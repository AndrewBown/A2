...
public class A3 
{
   ...
   private BST<Token> wordsByFreqDesc = new BST<Token>();
   private BST<Token> wordsByLength = new BST<Token>();
    ...
    */
	private void readFile() {
		...
		while (inp.hasNext()) 
		{
			...
			if (word.length() > 0) {
			Token wordToken = new Token(word);
				wordsByNaturalOrder.add(wordToken, "alphabetical");
				totalwordcount++;
			}
		}
   }
   ...
   /* Create the frequency and length lists. */
	private void createFreqLists() {
		wordsByFreqDesc.sortWordFreqMostToLeast(wordsByNaturalOrder);
   ...
   /* Remove stop words from the tree. */
	private void removeStop() {
		int listSize = wordsByNaturalOrder.size();
		for(String stopWord : stopwords) {
         
 			wordsByNaturalOrder.delete(stopWord);

			if (wordsByNaturalOrder.size() < listSize) {
				stopWordCount++;
				listSize = wordsByNaturalOrder.size();
			}
		}
 	}
